...
export function LandingPricingCards({ t, tierStrip, onGetStarted }: Props) {
  return (
    <div className="sa-lp-grid">
      {tierStrip.map((x) => (
        <PricingCard key={x.id} t={t} tier={x} onGetStarted={onGetStarted} />
      ))}
    </div>
  );
}
...
function PricingCard({
  ...
  const Icon = iconByTier[tier.id];
  const isPro = tier.id === "pro";
  const features = parseFeatures(t, tier.id);
  const price = t(`ref_tier_p_${tier.id}`, "");
  const sub =
    tier.id === "free"
  ...
  const titleId = `sa-lp-title-${tier.id}`;
  const ctaAria = `${btnLabel} — ${tier.name}`;

  const tilt = useTilt<HTMLElement>({ maxDeg: 4, perspective: 1100 });
  ...
  return (
    <article
      ...
      className={"sa-lp-card sa-lp-card--tilt" + (isPro ? " sa-lp-card--popular" : "")}
    ...
    >
      <div className="sa-lp-card__inner">
        <div className={"sa-lp-badge-row" + (isPro ? " sa-lp-badge-row--active" : "")}>
          {isPro && (
            <>
              <span className="sa-lp-sr-only">
                {t("ref_pricing_popular_plan", "Популярный тариф.")}
              </span>
        ...
        <div className="sa-lp-head">
          <div
            ...
            style={{
              ...
              background: `linear-gradient(145deg, ${tier.color}22, ${tier.color}08)`,
            }}
          >
            <Icon size={24} strokeWidth={1.75} color={tier.color} aria-hidden />
          </div>
          <div className="sa-lp-head-text">
            <h3 id={titleId} className="sa-lp-name">
              {tier.name}
            </h3>
            <p className="sa-lp-tag">{t(`ref_tier_d_${tier.id}`, "")}</p>
          </div>
        </div>
